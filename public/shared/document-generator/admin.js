            ...
            <div style="display:flex;justify-content:space-between;align-items:center;padding:12px 15px;border-bottom:1px solid var(--border);flex-wrap:wrap;gap:10px;">
                <div>
                    ...
                    <span style="font-size:0.8rem;color:var(--gray);margin-left:10px;">
                        ${category?.icon || '📄'} ${escapeHtml(category?.name || 'General')}
                    </span>
                    ...
                    ${doc.mimeType ? `<span style="font-size:0.7rem;color:var(--gray);margin-left:10px;">${doc.mimeType}</span>` : ''}
                </div>
...
async function uploadDocument() {
    ...
    try {
        ...
        const placeholders = extractedText.match(/\{([^}]+)\}/g) || [];
...
function renderCategories() {
    ...
    container.innerHTML = allCategories.map(cat => `
        <div class="category-card">
            ...
            <div style="font-size:0.8rem;color:var(--gray);">${escapeHtml(cat.name_hi || '')}</div>
        ...
        if (el) {
            ...
            el.innerHTML = `<option value="">-- Select Category --</option>` +
                allCategories.map(cat => 
                    `<option value="${cat.id}" ${cat.id === currentVal ? 'selected' : ''}>${escapeHtml(cat.icon || '📄')} ${escapeHtml(cat.name)}</option>`
                ).join('');
        }
    });
}
...
async function deleteCategory(catId) {
    ...
    if (docCount > 0) {
        alert(`Cannot delete: ${docCount} documents use this category. Reassign them first.`);
        return;
    }
