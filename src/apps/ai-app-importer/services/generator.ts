...
import appChatModulesSource from '@core/chat/appChatModules.ts?raw';
...
export interface GeneratedPackage {
  ...
  /** True if an updated src/core/chat/appChatModules.ts was included in `files`. */
  chatModuleRegistered: boolean;
...
`;
  ...
  // Auto-register the chat module so chatActions.ts is never left orphaned —
  // previously this required a manual edit to appChatModules.ts.
  let chatModuleRegistered = false;
  const hasChatActionsFile = updatedFiles.some((f) => f.path === `${appDir}/chatActions.ts`);
  const alreadyRegisteredInChat =
    appChatModulesSource.includes(`registerAppChatModule('${appSlug}'`) ||
    appChatModulesSource.includes(`registerAppChatModule("${appSlug}"`) ||
    appChatModulesSource.includes(`  '${appSlug}',`) ||
    appChatModulesSource.includes(`  "${appSlug}",`);

  if (hasChatActionsFile && !alreadyRegisteredInChat) {
    const registrationBlock = `\nregisterAppChatModule('${appSlug}', async () => {\n  const module = await import('@apps/${appSlug}/chatActions')\n  return module.chatModule\n})\n`;
    const updatedChatModules = appChatModulesSource.trimEnd() + '\n' + registrationBlock;
    updatedFiles.push({ path: 'src/core/chat/appChatModules.ts', content: updatedChatModules });
    chatModuleRegistered = true;
    notes.push(`Auto-registered the chat module for "${appSlug}" in src/core/chat/appChatModules.ts.`);
  } else if (alreadyRegisteredInChat) {
    notes.push(`A chat module for "${appSlug}" is already registered in src/core/chat/appChatModules.ts — left unchanged.`);
  }

  return { files: updatedFiles, manifest, appSlug, registryUpdated, loadersUpdated, chatModuleRegistered, notes };
}
...
function buildInstallReadme(pkg: GeneratedPackage): string {
  ...
  const chatModuleLine = pkg.chatModuleRegistered
    ? 'Replace src/core/chat/appChatModules.ts with the copy included in this zip (or merge the new registerAppChatModule(...) call by hand).'
    : 'This app already had a chat module registered, so no updated appChatModules.ts file is included.';
