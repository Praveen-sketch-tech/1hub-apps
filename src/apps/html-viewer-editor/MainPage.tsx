...
const minifyCode = () => {
let minified = code.replace(/<!--[\s\S]*?-->/g, ''); // Remove comments
minified = minified.replace(/>\s+</g, '><'); // Collapse spaces between tags
...
// Need to adjust for toolbar height (~50px)
...
style={{ flexBasis: layoutMode === 'editor-only' ? '100%' : splitRatio + '%', flexGrow: 0, flexShrink: 0 }}
>
<div className="flex flex-1 overflow-hidden">
{/ Line Numbers /}
<div
ref={lineNumbersRef}
className={"py-4 pr-3 pl-2 text-right select-none overflow-hidden font-mono text-sm " + (isDark ? "bg-[#1e1e1e] text-[#858585]" : "bg-gray-100 text-gray-500")}
style={{ fontSize: fontSize + 'px', minWidth: '40px' }}
>
{code.split('\n').map((_, i) => (
...
className={(layoutMode === 'split-h' ? "w-1.5 cursor-col-resize hover:w-2" : "h-1.5 cursor-row-resize hover:h-2") + " bg-blue-500/20 hover:bg-blue-500/50 transition-all z-10"}
/>
)}
{/* Preview Pane /}
{(layoutMode === 'split-h' || layoutMode === 'split-v' || layoutMode === 'preview-only') && (
<div 
className="flex-1 flex items-center justify-center bg-gray-200 dark:bg-black/80 relative overflow-hidden p-4"
>
<div 
className="bg-white rounded shadow-2xl transition-all duration-300 ease-in-out overflow-hidden relative"
style={previewStyle()}
>
{/ Using srcDoc for secure, cross-origin-free iframe rendering */}
<iframe
title="preview"
srcDoc={renderedCode}
...
className={"px-4 py-2 border-b-2 transition-colors " + (activeTab === 'stats' ? "border-blue-500 text-blue-500" : "border-transparent text-gray-500 hover:text-gray-400")}
>
📊 DOM Stats
</button>
<button
onClick={() => setActiveTab('validation')}
className={"px-4 py-2 border-b-2 transition-colors flex items-center gap-1 " + (activeTab === 'validation' ? "border-blue-500 text-blue-500" : "border-transparent text-gray-500 hover:text-gray-400")}
>
🚨 Validation
